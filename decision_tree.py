...
def resolve_base_dir():
    ...
    for candidate in candidates:
        if (candidate / "Data").exists() and (candidate / "Master Data").exists():
            return candidate
    raise FileNotFoundError("Could not find 'Data' and 'Master Data' folders next to the script or one level above.")


BASE_DIR = resolve_base_dir()

print("--- Step 1: Loading Datasets ---")
raw_data = pd.read_csv(BASE_DIR / 'Data' / 'dataset.csv')
severity_df = pd.read_csv(BASE_DIR / 'Master Data' / 'Symptom_severity.csv', header=None, names=['Symptom', 'Weight'])
description_df = pd.read_csv(BASE_DIR / 'Master Data' / 'symptom_Description.csv', header=None, names=['Disease', 'Description'])
precaution_df = pd.read_csv(BASE_DIR / 'Master Data' / 'symptom_precaution.csv', header=None)
