...
def load_and_preprocess_data():
    """
    Reads the CSV files from the repository `Data` and `Master Data` folders,
    cleans symptom strings and maps textual symptoms to numerical severity values.
    """
    base = resolve_base_dir()

    # 1. Load datasets
    df = pd.read_csv(base / 'Data' / 'dataset.csv')
    severity_df = pd.read_csv(base / 'Master Data' / 'Symptom_severity.csv', header=None, names=['Symptom', 'Weight'])

    # 2. Clean the symptom strings and create a mapping dictionary
    severity_df['Symptom'] = severity_df['Symptom'].astype(str).str.strip().str.replace(' ', '_')
