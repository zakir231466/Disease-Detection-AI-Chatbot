...
def train_selected_model(choice, x_train, y_train):
    models = {
        '1': ('Random Forest', RandomForestClassifier(n_estimators=300, random_state=42)),
        ...
        '4': ('Support Vector Machine', SVC(kernel='linear', probability=True, random_state=42)),
        '5': ('XGBoost', XGBClassifier(n_estimators=100, random_state=42, eval_metric='mlogloss', verbose=0))
    }
...
model_name, model = train_selected_model(model_choice, x_train, y_train)
...
def chatbot():
    ...
    print("\n" + "="*50)
    print(f"🤖 HealthCare ChatBot ({model_name})")
    print("="*50)
